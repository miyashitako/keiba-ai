...
__version__ = "2.0-margin_thresholds_override_param"
...
RECENT_FORM_PENALTY = [
    (8.0, 7.4),   # 加重平均着順 ≥ 8.0 → +7.4ポイント（旧+6.5）※暫定サンプルに基づく
    (6.0, 3.9),   # ≥ 6.0 → +3.9（旧+3.4）※暫定サンプルに基づく
    (4.5, 1.4),   # ≥ 4.5 → +1.4（旧+1.2）※暫定サンプルに基づく
]
...
# すること。
JRA_DIST_GOOD_FINISH_BONUS = {1: 5.9, 2: 5.8, 3: 5.2}
...
def calc_momentum_bonus(
    ...
    if curr_order > prev_order:          # 昇級
        ...
        if prev.margin >= 0.5:
            ...
            # 一部のみのため暫定値とする。
            return -2.3, "昇級(僅差勝ち)"   # 僅差勝ち昇級 → ペナルティ
    # 格下げ・同クラスはペナルティなし
    return 0.0, ""
