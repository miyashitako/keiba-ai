...
def calc_distance_aptitude_bonus(
    ...
    # ── ⑤ 芝ダ違いペナルティ
    # 今回の芝ダが指定されていて、かつ過去走に今回と同じ芝ダの走が1つもない場合
    surface_penalty = 0.0
    surface_label = ""
    if target_surface and past_races:
        ...
        if not same_surf:
            ...
            surface_label = f"{other_surf}→{target_surface}転向:-1.0"
...
def calc_phase1(
    ...
    # ── 近走不振ペナルティ（v1.1：age_limited対応）
    form_pen, form_label = calc_recent_form_penalty(
        past_races_all, target_distance, target_surface,
        age_limited=age_limited,
    )
    if form_pen > 0:
        result.phase1_score = round(result.phase1_score + form_pen, 3)
        result.ability_avg  = round(result.ability_avg  + form_pen, 3)
        result.note = (result.note + f" [{form_label}]").strip()
