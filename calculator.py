...
__version__ = "1.7-jra_recalibration_v5_large_combined_sample"
...
RECENT_FORM_PENALTY = [
    (8.0, 6.3),   # 加重平均着順 ≥ 8.0 → +6.3ポイント（旧+6.0）
    (6.0, 3.3),   # ≥ 6.0 → +3.3（旧+3.1）
    (4.5, 1.2),   # ≥ 4.5 → +1.2（旧+1.1）
]
...
JRA_DIST_GOOD_FINISH_BONUS = {1: 5.2, 2: 4.9, 3: 4.4}
...
def calc_momentum_bonus(
    ...
    if curr_order > prev_order:          # 昇級
        ...
        if prev.margin >= 0.5:
            ...
            return -1.4, "昇級(僅差勝ち)"   # 僅差勝ち昇級 → ペナルティ
    # 格下げ・同クラスはペナルティなし
    return 0.0, ""
